...
# SVD paths
svd_computer_path = 'util/datasets/Schwartz_mouse_v1/svd/Schwartz_mouse_svd_computer.pickle'
mean_path = 'util/datasets/Schwartz_mouse_v1/svd/Schwartz_mouse_mean.pickle'
...
# to calculate angle between 2 3D vectors
...
def single_alignment_2D(trajectory: np.ndarray, t_matrix, r_matrix):
    ...
    for i in range(int(seq_shape[1] / 3)):
        joint = np.array([trajectory[:,3 * i], trajectory[:,3 * i + 1], np.ones([seq_shape[0]])])
...
def transform_to_svd_components(data,
    ...
    mouse_rotation = np.arctan2(
        data[:, 0, 0] - data[:, 4, 0], data[:, 0, 1] - data[:, 4, 1])
    ...
    centered_data = centered_data.reshape((-1, 16))
